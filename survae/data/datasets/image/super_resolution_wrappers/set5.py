...
class Set5Dataset(data.Dataset):
    ...
    """
    def __init__(self, resize_hw=None, root=DATA_PATH, split='test', transform=None, sr_scale_factor=4, bicubic=False, repeats=1):
        super(Set5Dataset, self).__init__()
        
        assert split in {'test'}
        self.root = os.path.expanduser(root)
        self.raw_data_folder = os.path.join(self.root, 'Set5/')
        self.split = split
