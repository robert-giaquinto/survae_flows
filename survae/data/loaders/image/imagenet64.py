from survae.data.datasets.image import UnsupervisedImageNet64Dataset, SuperResolutionImageNet64Dataset
...
class ImageNet64(TrainTestLoader):
    ...
    def __init__(self, root=DATA_PATH, download=True, num_bits=8, pil_transforms=[], conditional=False, super_resolution=False, sr_scale_factor=4, resize_hw=None):
        ...
        # Load data
        if super_resolution:
            self.train = SuperResolutionImageNet64Dataset(root, train=True, transform=Compose(trans_train), download=download, sr_scale_factor=sr_scale_factor)
            self.test = SuperResolutionImageNet64Dataset(root, train=False, transform=Compose(trans_test), sr_scale_factor=sr_scale_factor)
        else:
            self.train = UnsupervisedImageNet64Dataset(root, train=True, transform=Compose(trans_train), download=download)
            self.test = UnsupervisedImageNet64Dataset(root, train=False, transform=Compose(trans_test))
