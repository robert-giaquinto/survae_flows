...
from survae.transforms import ScalarAffineBijection, Squeeze2d, Conv1x1, ConditionalConv1x1, Slice, SimpleMaxPoolSurjection2d, ActNormBijection2d

from model.coupling import SRCoupling
...
class ContextInit(nn.Module):
    ...
    def forward(self, context):
        context, ldj = self.dequant(context)
        context, ldj = self.shift(context)
        return context
    ...
    def __init__(self, data_shape, cond_shape, num_bits,
        ...
        for scale in range(num_scales):
            for step in range(num_steps):
                ...
                transforms.append(ConditionalConv1x1(cond_shape=cond_shape, num_channels=current_shape[0]))
                transforms.append(SRCoupling(x_size=cond_shape, y_size=current_shape, mid_channels=coupling_channels, depth=coupling_depth))
