...
class StudentExperiment(BaseExperiment):
    ...
    def __init__(self, args,
        ...
        arch_id = f"{aug_or_abs}_k{args.num_flows}_h{hidden}_{'affine' if args.affine else 'additive'}{'_actnorm' if args.actnorm else ''}"
        ...
        if args.parallel == 'dp':
            ...
            teacher = DataParallelDistribution(teacher)
        ...
        with torch.no_grad():
            ...
            for i in range(iters_per_epoch):
                ...
                print('Evaluating. Epoch: {}/{}, Datapoint: {}/{}, NLL: {:.3f}'.format(
                    epoch+1, self.args.epochs, loss_count, self.args.train_samples, loss_sum/loss_count), end='\r')
            print('')
        return {'nll': loss_sum/loss_count}
    ...
    def plot_fn(self):
        ...
        for i, x in enumerate(self.context_permutations()):
            with torch.no_grad():
                ...
                x = x.expand((y.size(0), 2))
                samples = self.model.sample(context=x, temperature=1.5)
                samples = samples.cpu().numpy()
            ...
            xv, yv = torch.meshgrid([torch.linspace(bounds[0][0], bounds[0][1], self.args.grid_size), torch.linspace(bounds[1][0], bounds[1][1], self.args.grid_size)])
            y = torch.cat([xv.reshape(-1,1), yv.reshape(-1,1)], dim=-1).to(self.args.device)
            x = x[0].expand((y.size(0), 2))
            with torch.no_grad():
                logprobs = self.model.log_prob(y, context=x)
                logprobs = logprobs - logprobs.max().item()
                probs = 0.2 * logprobs.exp()

            plt.figure(figsize=(self.args.pixels/self.args.dpi, self.args.pixels/self.args.dpi), dpi=self.args.dpi)
            plt.pcolormesh(xv, yv, probs.reshape(xv.shape))
            plt.xlim(bounds[0])
            plt.ylim(bounds[1])
            plt.axis('off')
            print('Range:', probs.min().item(), probs.max().item())
            print('Limits:', 0.0, self.args.clim)
            plt.clim(0,self.args.clim)
            plt.savefig(os.path.join(plot_path, f'fixed_context_flow_density_{i}.png'), bbox_inches='tight', pad_inches=0)

    def _preprocess_context(self, y):
        #x = y[:, self.context_id:]
        #r = torch.rand_like(y)
        #x = torch.masked_select(y, r == r.max(dim=1).values.view(self.args.batch_size, 1)).view(self.args.batch_size, 1)
        #x = y.masked_fill_(r == r.max(dim=1).values.view(self.args.batch_size, 1), 0.0)

        #x = y[:, i].view(y.size(0), 1)
        #x = y
        #x[:, i] = 0.0
        y = (y - y.min().item()) / (y.max().item() - y.min().item())
        x = torch.round(y)
        return x
    ...
    def context_permutations(self, ymin=-10, ymax=10):
        ...
        xv, yv = torch.meshgrid([torch.linspace(bounds[0][0], bounds[0][1], self.args.grid_size), torch.linspace(bounds[1][0], bounds[1][1], self.args.grid_size)])
        y = torch.cat([xv.reshape(-1,1), yv.reshape(-1,1)], dim=-1).to(self.args.device)
        x = self._preprocess_context(y).unique(sorted=True, dim=0)
        return x
